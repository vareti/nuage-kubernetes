...
func NewNuageEtcdClient(nkmConfig *config.NuageKubeMonConfig) (*NuageEtcdClient, error) {
	...
	if nuageetcd.scaleUpThreshold, err = strconv.Atoi(nkmConfig.EtcdClientConfig.ScaleUpThreshold); err != nil {
		glog.Errorf("converting subnet scale up threshold(%s) to int failed: %v", nkmConfig.EtcdClientConfig.ScaleUpThreshold, err)
		glog.Warningf("using default scale up value 75%")
		nuageetcd.scaleUpThreshold = 75
	}
	if nuageetcd.scaleDownThreshold, err = strconv.Atoi(nkmConfig.EtcdClientConfig.ScaleDownThreshold); err != nil {
		glog.Errorf("converting subnet scale down threshold(%s) to int failed: %v", nkmConfig.EtcdClientConfig.ScaleDownThreshold, err)
		glog.Warningf("using default scale down value 25%")
		nuageetcd.scaleDownThreshold = 25
	}
...
func (nuageetcd *NuageEtcdClient) DeAllocateSubnetFromPod(data *api.EtcdPodMetadata) ([]*api.EtcdSubnetMetadata, error) {
	...
	f := func() (bool, error) {
		...
		if noOfSubnets != 1 && ACTIVEIPCount*100 < noOfSubnets*nuageetcd.maxIPCount*nuageetcd.scaleDownThreshold {
			for _, kv := range subnetResp.Kvs {
				if err := json.Unmarshal(kv.Value, snet); err != nil {
					glog.Errorf("unmarshal kv pair(%s, %s) to struct failed: %v", kv.Key, kv.Value, err)
				} else {

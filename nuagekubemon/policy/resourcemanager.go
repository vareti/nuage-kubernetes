		...
		for _, ingressRule := range pe.Policy.Ingress {
			for _, from := range ingressRule.From {
				if from.NamespaceSelector != nil {
					sourceNamespaceSelector, err := unversioned.LabelSelectorAsSelector(from.NamespaceSelector)
					if err != nil {
						glog.Errorf("Couldn't set source namespaceSelector labels with error: %v", err)
						continue
					}
					//get pods for this selector and add them to pg.
					namespaces, err := rm.clusterClientCallBacks.FilterNamespaces(&kapi.ListOptions{LabelSelector: sourceNamespaceSelector, FieldSelector: fields.Everything()})
					if err != nil {
						glog.Errorf("Couldn't retrieve namespace matching labels %s because of error: %s", sourceNamespaceSelector.String(), err)
						continue
					}
